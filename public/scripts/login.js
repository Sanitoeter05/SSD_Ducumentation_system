function login(){
    ...
    if (u_email, u_pass){
        payload = {'u_email': u_email, 'u_pass': u_pass, "login": "1"}
        fetch('/data/user', { method : 'POST',  headers: {'Content-Type': 'application/json'}, body: JSON.stringify(payload)}).then(response => response.json()).then( (result) => {
